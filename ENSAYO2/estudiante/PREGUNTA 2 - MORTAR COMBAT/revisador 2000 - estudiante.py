...
for alumno in alumnos:
    ...
    if not formato:
        ...
        while i < len(testE):
            ...
            try:
                try :
                    with open('./estudiantes/'+alumno,"r",encoding="utf-8") as a:
                        code_estudiante = a.read()
                except:
                    with open('./estudiantes/'+alumno) as a:
                        code_estudiante = a.read()

                try :
                    with open("main_base.txt","r", encoding="utf-8") as a:
                        code_base = a.read()
                except:
                    with open("main_base.txt","r") as a:
                        code_base = a.read()

                code_estudiante = code_estudiante + code_base

                with open("main_base.py","w", encoding="utf-8") as a:
                        a.write(code_estudiante)
